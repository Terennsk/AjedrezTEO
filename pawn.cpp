...
void Pawn::moves()
{
    ...
    if(this->getSide() == "blancos")  {
        if(col > 0 && row > 0 && game->collection[row-1][col-1]->getChessPieceColor() == "negros") {
            location.append(game->collection[row-1][col-1]);
            boxSetting(location.last());
        }
        if(col < 7 && row > 0 && game->collection[row-1][col+1]->getChessPieceColor() == "negros") {
            location.append(game->collection[row-1][col+1]);
            boxSetting(location.last());
        }
        ...
        if(col > 0 && row < 7 && game->collection[row+1][col-1]->getChessPieceColor() == "blancos") {//izquierda
            location.append(game->collection[row+1][col-1]);
            boxSetting(location.last());
        }
        if(col < 7 && row <  7 && game->collection[row+1][col+1]->getChessPieceColor() == "blancos") {//derecha
            location.append(game->collection[row+1][col+1]);
            boxSetting(location.last());
        }
